...
App.prototype.renderDashboard = function (activeTabName) {
    ...
    var params = {tags: ["BooksNTHU"], page: '1', per_page: '100'};

    QB.users.get(params, function(err, result) {
        if (result) {
            console.log("This is USER DATA: ", result.items);
            for (var i = 0; i < result.items.length; i++) {
              localStorage.setItem(result.items[i].user.id, result.items[i].user.facebook_id);
            }
        }
    });
    ...
    logoutBtn.addEventListener('click', function () {
        console.log("dashboard close in app.js");
        document.querySelector('.dashboard').style.display = "none";
        document.querySelector('#chatroom').style.display = "inline";
        document.querySelector('#chatroom').innerHTML = " CHATROOM "

    });

    if (document.getElementById('chatroom').innerHTML.indexOf("Loading") != -1) {
