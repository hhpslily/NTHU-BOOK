...
<?php
  ...
  include '../api/config/db.php';
	...
	if (isset($_GET['search']) && !empty($_GET['search'])) {
		$search = $_GET['search'];
		$sql_cond = $sql_cond . "AND Books.b_name LIKE \"$search\" OR Books.b_dept LIKE \"$search\" ";
	}
    ...
    <body>
		...
		<div class="search_bar container">
			<div class="row">
				...
				<!-- <button class="col-md-4" id="searchBtn" name="button" type="button">搜尋</button> -->
			</div>
		...
		<div class="book_list container">
			...
			<nav aria-label="Page navigation example">
				...
				<?php
					...
					// echo " <a style=\"font-size: 30px; border-bottom: 2px solid; padding: 0 3px;\" href='{$_SERVER['PHP_SELF']}?curPage=1'><<</a> ";
					$prevPage = $curPage - 1;
					echo "<li class=\"page-item\"><a class=\"page-link\" href=\"{$_SERVER['PHP_SELF']}?id=$id&curPage=$prevPage&dept=$dept&search=$search\"><</a></li>";
					//echo " <a style=\"font-size: 30px; border-bottom: 2px solid; padding: 0 3px;\" href='{$_SERVER['PHP_SELF']}?curPage=$prevPage'><</a> ";
